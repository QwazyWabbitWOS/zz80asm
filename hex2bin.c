...
#define CKS_16BE  2
  ...
  //Param = 1;
	...
	for (Param = 1; Param < (unsigned int)argc; ++Param)
	{
		c = p = argv[Param];
		//#ifdef __linux__
			  /* We don't accept an option beginning with a '/' because it could be a file name. */
		...
		*/
		...
		if (!feof(Filin)) {
			...
			if (Max_Length_Setted == TRUE) {
				//MaxLength not reached yet
				if (Highest_Address < (Starting_Address + Max_Length - 1)) {
					do {
						++Highest_Address;
						Memory_Block[Highest_Address] = PadByte;
					} while (Highest_Address < (Starting_Address + Max_Length - 1));
				}
			}
		}
		//Loop_Finished = (!feof (Filin)) || ((Max_Length_Setted==TRUE)?(Highest_Address < (Starting_Address + Max_Length)):(FALSE));
	} while (!feof(Filin));
	/*-----------------------------------------------------------------------------*/
